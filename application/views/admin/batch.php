<div class="boxed">
				...
				<div id="page-content">
					...
					<div class="panel">
						...
						<div class="pull-left col-md-6">
							<h3 class="panel-title">Users Batch</h3>
						</div>
							<div class="pull-right col-md-6" style="text-align: right; margin-top: 10px;">
								<button data-target="#add-batch-modal" data-toggle="modal" class="add_batch_model btn btn-success">Add Batch</button>
							</div>
						</div>
		...
		<!--===================================================-->
		<div class="modal fade" id="add-batch-modal" role="dialog" tabindex="-1" aria-labelledby="demo-default-modal" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<!--Modal body-->
					<form action="<?php echo base_url('admin/save_user_batch') ?>" method = "post" id="add-batch-form" class="form-horizontal">
						<div class="modal-body">
							<div class="row">
								  <div class="col-md-12">
								    
								      <div class="form-group">
								        <label class="col-md-4 control-label" for="name">Total Trip Post</label>
								        <div class="col-md-4">
								          <input id="total_post" name="total_post" placeholder="Total Trip Post" class="form-control input-md" type="text" required>
								          <span class="help-block"><small></small></span> </div>
								      </div>

								      <div class="form-group">
		                                <label class="col-md-4 control-label" for="name">Batch </label>
		                                <div class="col-md-6">
		                                  <input id="batch_picture" name="batch_picture" class="form-control input-md" type="file" required>
		                                  <div class="clearfix"></div>
		                                  <img class="add_batch_pic" width="100"/>
		                                  <span class="help-block"><small></small></span> </div>
		                              </div>
						...
						<div class="modal-footer">
							...
							<button class="btn btn-info">Save Batch</button>
						</div>
					</form>
				</div>
			</div>
		...
		<div class="modal fade" id="edit-batch-modal" role="dialog" tabindex="-1" aria-labelledby="demo-default-modal" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<!--Modal body-->
					<form action="<?php echo base_url('admin/edit_user_batch') ?>" method = "post" id="edit-batch-form" class="form-horizontal">
							...
							<div class="row">
								  <div class="col-md-12">
								      <div class="form-group">
								        <label class="col-md-4 control-label" for="name">Total Trip Post</label>
								        <div class="col-md-4">
								          <input id="total_post" name="total_post" placeholder="Total Trip Post" class="form-control input-md" type="text">
								          <span class="help-block"><small></small></span> </div>
								      </div>
								      ...
								      <div class="form-group">
		                                <label class="col-md-4 control-label" for="name">Batch </label>
		                                <div class="col-md-6">
		                                  <input id="batch_picture" name="batch_picture" class="form-control input-md" type="file">
		                                  <div class="clearfix"></div>
		                                  <img class="add_batch_pic" width="100"/>
		                                  <span class="help-block"><small></small></span> </div>
		                              </div>
									<input type="hidden" name="batch_id" id="batch_id" value="">
									<input type="hidden" name="batch_img" id="batch_img" value="">
								  </div>
								</div>
						</div>
						...
						<div class="modal-footer">
							<button data-dismiss="modal" class="btn btn-default" type="button">Close</button>
							<button class="btn btn-info">Edit Batch</button>
						</div>
					</form>
				</div>
			</div>
  ...
  <script src="//cdn.datatables.net/1.10.2/js/jquery.dataTables.min.js"></script>
    ...
    <script>
	    $(document).ready(function(){
	    	var oTable= $('#batch_table').DataTable({
				"aoColumns": [
					{ "iDataSort": 1 },
					{ "iDataSort": 2 },
					{ "bSortable": false },
				...
				"responsive": true,
				"iDisplayLength":10,
				"ajax": "<?php echo base_url('admin/manage_batch'); ?>",
			});
	    	...
	    	$(document).on("submit","#add-batch-form,#edit-batch-form",function(e){ 
				e.preventDefault();
				var elem = $(this);
		        ...
		        $.ajax({
		            ...
		            success: function (data) {
						if(!data.error){
							var success = '<div class="alert alert-success"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><p>'+data.message+'</p></div>';
							elem.prepend(success);
							elem.find("#batch_picture").val("").clone(true);

							if(_id == "add-batch-form"){
	                            elem.find("img.add_batch_pic").attr('src', '');
								$('#'+_id)[0].reset();
	                        }
		        ...
		        $.ajax({
		            ...
		            success: function (data) {
		                if (!data.error) {
		                    ...
		                    var _model = $("#edit-batch-modal");
		                    	_model.find("#total_post").val(details.total_post);
		                    	_model.find("#batch_id").val(details.batch_id);
		                    	_model.find("#batch_img").val(details.alot_bacth);
		                    	_model.find("img.add_batch_pic").attr("src", "<?php echo base_url("uploads/") ?>"+details.alot_bacth);
		                    _model.modal("show");
		                } else {
		                    alert(data.message);
		                }
			...
			$("#batch_picture").on('change', function () {
		        var fileExtension = ['jpeg', 'jpg', 'png', 'gif', 'bmp'];
		        if ($.inArray($(this).val().split('.').pop().toLowerCase(), fileExtension) == -1) {
		            var error = "Only " + fileExtension.join(', ') + " formats are allowed.";
		            $(this).next("span.help-block").text(error);
		            $("#batch_picture").closest("div.form-group").find("img.add_batch_pic").attr('src', "");
		            $("#batch_picture").closest("div.form-group").addClass("has-error");
		            $(this).val('').clone(true)
		        } else {
		            readURL(this);
		        }
	...
	function readURL(input) {
	    if (input.files && input.files[0]) {
	        ...
	        reader.onload = function (e) {
	            $("#batch_picture").closest("div.form-group").find("img.add_batch_pic").attr('src', e.target.result);
	            $("#batch_picture").closest("div.form-group").removeClass(" has-error");
	            $("#batch_picture").closest("div.form-group").find("span.help-block").text("");
	        }
