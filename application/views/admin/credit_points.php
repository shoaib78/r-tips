<div class="boxed">
				...
				<div id="page-content">
					...
					<div class="panel">
						...
						<div class="pull-left col-md-6">
							<h3 class="panel-title">Credit Points</h3>
						</div>
							<div class="pull-right col-md-6" style="text-align: right; margin-top: 10px;">
								<button data-target="#add-points-modal" data-toggle="modal" class="credit_points_modal btn btn-success">Add Credit Points</button>
							</div>
						</div>
						<div class="panel-body">
							<table id="credit_points" class="table table-striped table-bordered" cellspacing="0" width="100%">
								<thead>
									<tr>
										<th>Credit Points</th>
										<th>Total Trip Pictures</th>
		...
		<!--===================================================-->
		<div class="modal fade" id="add-points-modal" role="dialog" tabindex="-1" aria-labelledby="demo-default-modal" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-header">
						...
						<h4 class="modal-title">Add Credit Points</h4>
					</div>

					<!--Modal body-->
					<form action="<?php echo base_url('admin/save_credit_points') ?>" method = "post" id="add_credit_points" class="form-horizontal">
						<div class="modal-body">
							<div class="row">
								  <div class="col-md-12">
								    
								      <div class="form-group">
								        <label class="col-md-4 control-label" for="name">Credit Points</label>
								        <div class="col-md-4">
								          <input id="credit_point" name="credit_point" placeholder="Points" class="form-control input-md" type="text" required>
								          <span class="help-block"><small></small></span> </div>
								      </div>

								      <div class="form-group">
								        <label class="col-md-4 control-label" for="name">Total Trip Picture</label>
								        <div class="col-md-4">
								          <input id="total_picture" name="total_picture" placeholder="Trip Picture" class="form-control input-md" type="text" required>
								          <span class="help-block"><small></small></span> </div>
								      </div>
						...
						<div class="modal-footer">
							...
							<button class="btn btn-info">Save Credit Points</button>
						</div>
					</form>
				</div>
			</div>
		...
		<div class="modal fade" id="edit-points-modal" role="dialog" tabindex="-1" aria-labelledby="demo-default-modal" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-header">
						...
						<h4 class="modal-title">Edit Credit Points</h4>
					</div>

					<!--Modal body-->
					<form action="<?php echo base_url('admin/edit_credit_points') ?>" method = "post" id="edit_credit_points" class="form-horizontal">
						...
						<div class="modal-body">
							<div class="row">
								  <div class="col-md-12">
									...
									<input type="hidden" name="credit_points_id" id="credit_points_id" value="">
								  </div>
								</div>
						</div>
						...
						<div class="modal-footer">
							<button data-dismiss="modal" class="btn btn-default" type="button">Close</button>
							<button class="btn btn-info">Edit Credit Points</button>
						</div>
					</form>
				</div>
			</div>
  ...
  <script src="//cdn.datatables.net/1.10.2/js/jquery.dataTables.min.js"></script>
    ...
    <script>
	    $(document).ready(function(){
	    	var oTable= $('#credit_points').DataTable({
				"aoColumns": [
					{ "iDataSort": 1 },
					{ "iDataSort": 2 },
					{ "bSortable": false },
				...
				"responsive": true,
				"iDisplayLength":10,
				"ajax": "<?php echo base_url('admin/manage_credit_points'); ?>",
			});
	    	...
	    	$(document).on("submit","#add_credit_points,#edit_credit_points",function(e){ 
				e.preventDefault();
				var elem = $(this);
				...
				$.ajax({
					...
					success: function (data) {
						if(!data.error){
							...
							if(_id == "add_credit_points"){
	                            $('#add_credit_points')[0].reset();
	                        }
                            $('#credit_points').DataTable().ajax.reload();
							setTimeout(function(){
			...
			$(document).on("click",".delete_credit_points",function(e){ 
				e.preventDefault();
		        var elem = $(this);
			...
			$(document).on("click",".edit_credit_points",function(e){ 
				e.preventDefault();
		        var elem = $(this);
		        var url = elem.attr("href");
		        ...
		        $.ajax({
		            ...
		            success: function (data) {
		                if (!data.error) {
		                    ...
		                    var _model = $("#edit-points-modal");
		                    _model.find("#credit_point").val(details.credit_point);
		                    _model.find("#total_picture").val(details.total_picture);
		                    _model.find("#credit_points_id").val(details.credit_points_id);
		                    _model.modal("show");
		                } else {
		                    alert(data.message);
		                }
